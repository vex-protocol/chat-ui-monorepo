...
Spire is a single-process Node.js server (Express 5 + `ws` library) backed by SQLite via Kysely. It serves both HTTP REST endpoints and a WebSocket endpoint on the same port (`16777`). All state lives either in SQLite on disk or in two in-process `Map` objects.
...
- No FUSE, no filesystem tricks — just a different Kysely dialect

This is the lowest-friction path to multi-server SQLite. Replace `better-sqlite3` with `@libsql/client` and change the Kysely dialect. All existing queries work unchanged because libSQL is SQLite-compatible.
...
- **PostgreSQL**: Complete the existing `DB_TYPE=postgres` path in `createDb`. Kysely makes this a dialect swap. Connection pooling handles concurrent writes.
...
See also: [infrastructure.md](infrastructure.md) for provider selection, [architecture.md](../reference/architecture.md) for the current server structure, [websocket.md](../reference/websocket.md) for connection lifecycle.
