...
Story metadata is written once in `*.stories-shared.ts` files co-located with each Mitosis source, and thin per-framework wrapper files (generated by `scripts/gen-story-wrappers.ts`) add only the `component` import. See `docs/packages/packages.md` for the full file layout and code examples.
...
See also: [packages-store-ui.md](../packages/packages-store-ui.md) for the Mitosis file layout and Storybook architecture, [platform-strategy.md](platform-strategy.md) for how desktop and mobile consume shared components.
