...
| **argon2id** | Memory-hard password hashing algorithm used by the new server. Replaces PBKDF2-SHA512 (1,000 iterations) from the original. Resistant to GPU/ASIC brute-force attacks | [auth-comparison.md](explanation/auth-comparison.md) |
...
| **double ratchet** | Session key advancement protocol where each message derives a new key from the previous one. The original vex-desktop implemented this with HMAC verification and auto-healing. The new client uses fresh ephemeral keys per message instead | [journeys.md](ops/journeys.md) |
...
| **familiar** | A user you have previously exchanged messages with. The original server tracked familiars server-side via `users.familiars()`. The new server does not — the `$familiars` atom is populated client-side from message history | [journeys.md](ops/journeys.md) |
...
| **permission** | A database record granting a user access to a server resource. Has a `powerLevel` (0 = member, 1 = default member, 50 = moderator, 100 = admin). Created when joining via invite | [architecture.md](reference/architecture.md) |
...
| **SPK (server private key)** | The server's own NaCl Ed25519 signing key pair. The public half is shared with clients for verifying server-signed payloads. The private half is the `SPK` environment variable. Separate from `JWT_SECRET` | [vex-overview.md](vex-overview.md) |
...
See also: [vex-overview.md](vex-overview.md) for the full cryptographic protocol, [architecture.md](reference/architecture.md) for server structure.
