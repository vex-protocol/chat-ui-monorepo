...
### Phase D — Mobile Expo Prebuild

Migrate `apps/mobile` from bare RN 0.84.1 to Expo Prebuild (CNG + Expo SDK).

**Why Prebuild (not managed):**
- `@notifee/react-native` → no drop-in Expo equivalent; `expo-notifications` lacks Android channel granularity
- `react-native-keychain` → service-name scoping that `expo-secure-store` doesn't provide
- Metro workspace config composes cleanly with Expo's default

**Steps:**
1. Audit Expo SDK ↔ RN 0.84.1 compatibility; likely pin RN to Expo SDK 53 baseline (RN 0.79) OR use Expo canary
2. `pnpm --filter @vex-chat/mobile add expo`
3. Expand `app.json` from 4-line bare to full Expo config with `expo.plugins: [@notifee/react-native/expo-plugin, react-native-keychain]`, `newArchEnabled: true`, `infoPlist` permissions, `android.permissions`
4. Compose `expo/metro-config` `getDefaultConfig()` into existing `metro.config.js` while preserving `watchFolders` + `unstable_enableSymlinks` + `unstable_enablePackageExports`
5. Switch `babel.config.js` to `babel-preset-expo`
6. Update `package.json` scripts: `expo start` / `expo run:ios` / `expo run:android` / `expo prebuild`
7. Verify `index.js` polyfill ordering (`InitializeCore` → `get-random-values` → `fast-text-encoding`)
8. Back up `ios/`+`android/` to branch, `rm -rf ios android`, `npx expo prebuild --clean`
9. Rebuild + smoke test: auth flow, keychain store/retrieve, notifee foreground + background
10. Delete `react-native.config.js` (font assets via `expo.assetBundlePatterns` or `expo-font`)
...
| RN 0.84.1 ahead of Expo SDK | Pin RN to Expo baseline OR use Expo canary tracking 0.84 |
...
2. **Expo SDK target** — RN pin to SDK 53 baseline (RN 0.79) OR ride Expo canary to keep RN 0.84.1?
