...
export function ChannelListScreen({ route, navigation }: { route: any; navigation: any }) {
  ...
  function renderChannel({ item }: { item: IChannel }) {
    return (
      <TouchableOpacity
        style={styles.row}
        onPress={() => navigation.navigate('Channel', { channelID: item.channelID, channelName: item.name, serverID })}
      >
...
const styles = StyleSheet.create({
  ...
  row: { flexDirection: 'row', alignItems: 'center', gap: 8, padding: 14, borderBottomWidth: 1, borderBottomColor: '#2a2a2a' },
