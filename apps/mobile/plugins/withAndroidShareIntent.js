...
function withShareIntentFilters(config) {
    return withAndroidManifest(config, (cfg) => {
        ...
        const filters = [
            ...
            ["android.intent.action.SEND", "video/*"],
            ["android.intent.action.SEND", "audio/*"],
            ["android.intent.action.SEND", "application/*"],
            ["android.intent.action.SEND", "text/plain"],
            ["android.intent.action.SEND", "*/*"],
            ["android.intent.action.SEND_MULTIPLE", "image/*"],
            ["android.intent.action.SEND_MULTIPLE", "video/*"],
            ["android.intent.action.SEND_MULTIPLE", "audio/*"],
            ["android.intent.action.SEND_MULTIPLE", "application/*"],
...
class VexShareIntentModule(
    ...
    fun clearShare(id: String, promise: Promise) {
        ...
        if (cachedShare?.getString("id") == id) {
            deleteCachedShareFiles(cachedShare)
            cachedShare = null
        }
        promise.resolve(null)
    }
    ...
    private fun deleteCachedShareFiles(share: WritableMap?) {
        val items = share?.getArray("items") ?: return
        val shareDir = File(reactContext.cacheDir, "vex-shares")
        val shareDirPath = shareDir.canonicalPath
        for (index in 0 until items.size()) {
            val uriString = items.getMap(index)?.getString("uri") ?: continue
            val path = Uri.parse(uriString).path ?: continue
            val file = File(path)
            val filePath = file.canonicalPath
            if (filePath == shareDirPath || !filePath.startsWith("\${shareDirPath}\${File.separator}")) {
                continue
            }
            file.delete()
        }
