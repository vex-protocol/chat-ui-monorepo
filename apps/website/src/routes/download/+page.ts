...
function classifyAsset(name: string): { label: string; platform: string } | null {
	const n = name.toLowerCase();
	if (n.endsWith('.dmg')) return { label: 'macOS (.dmg)', platform: 'macos' };
	if (n.endsWith('.exe') || n.endsWith('.msi')) return { label: 'Windows (.exe)', platform: 'windows' };
	if (n.endsWith('.appimage')) return { label: 'Linux (.AppImage)', platform: 'linux' };
