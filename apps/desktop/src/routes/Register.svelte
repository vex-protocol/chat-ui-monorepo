<script lang="ts">
  ...
  async function handleRegister(e: SubmitEvent) {
    ...
    if (usernameError) { errors = { username: usernameError }; return }
    if (password.length < 6) { errors = { password: 'Password must be at least 6 characters' }; return }
    if (password !== confirm) { errors = { confirm: 'Passwords do not match' }; return }
  ...
  <div class="auth-card">
    ...
    <form class="auth-form" onsubmit={handleRegister}>
      <div class="auth-form__field">
        ...
        <input id="username" type="text" autocomplete="username" placeholder="pick a username" bind:value={username} disabled={loading} required />
        {#if errors.username}<span class="field-error">{errors.username}</span>{/if}
      </div>

      <div class="auth-form__field">
        <label for="password">Password</label>
        <input id="password" type="password" autocomplete="new-password" placeholder="••••••••" bind:value={password} disabled={loading} required />
        {#if errors.password}<span class="field-error">{errors.password}</span>{/if}
      </div>

      <div class="auth-form__field">
        <label for="confirm">Confirm Password</label>
        <input id="confirm" type="password" autocomplete="new-password" placeholder="••••••••" bind:value={confirm} disabled={loading} required />
