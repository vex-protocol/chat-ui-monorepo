...
<div class="home">
  <div class="home__card">
    ...
    <p class="home__subtitle">Create a server or join one with an invite link.</p>

    <button class="home__create" onclick={() => { showCreate = true }}>
      Create a Server
    </button>
    ...
    <form class="home__join" onsubmit={joinViaInvite}>
      <input
        ...
        autocomplete="off"
      />
      <button class="home__join-btn" type="submit" disabled={!inviteInput.trim() || joining}>
...
<style>
  ...
  .home__join-btn:hover:not(:disabled) { opacity: 0.9; }
  .home__join-btn:disabled { opacity: 0.5; cursor: not-allowed; }
